...
void Simulate::Initialize() {
  ...
  // creating targets
  m_targets.push_back(
      simulateTarget(this, arma::fvec({0.f, 0.f}), arma::fvec({0.f, -10.f}),
                     std::make_pair(1 * m_delta_t, 70 * m_delta_t)));                     
  m_targets.push_back(
      simulateTarget(this, arma::fvec({400.f, -600.f}), arma::fvec({-10.f, 5.f}),
                     std::make_pair(1 * m_delta_t, 101 * m_delta_t)));                     
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-800.f, -200.f}), arma::fvec({20.f, -5.f}),
                     std::make_pair(1 * m_delta_t, 70 * m_delta_t)));
                     
                     
                     
  m_targets.push_back(
      simulateTarget(this, arma::fvec({400.f, -600.f}), arma::fvec({-7.f, -4.f}),
                     std::make_pair(20 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({400.f, -600.f}), arma::fvec({-2.5f, 10.f}),
                     std::make_pair(20 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({0.f, 0.f}), arma::fvec({7.5f, -5.f}),
                     std::make_pair(20 * m_delta_t, 101 * m_delta_t)));
  ...
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-800.f, 200.f}), arma::fvec({12.f, 7.f}),
                     std::make_pair(40 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-200.f, 800.f}), arma::fvec({15.f, -10.f}),
                     std::make_pair(20 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-800.f, -200.f}), arma::fvec({3.f, 15.f}),
                     std::make_pair(60 * m_delta_t, 101 * m_delta_t)));
    
    
                     
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-200.f, 800.f}), arma::fvec({-3.f, -15.f}),
                     std::make_pair(60 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({0.f, 0.f}), arma::fvec({-20.f, -15.f}),
                     std::make_pair(80 * m_delta_t, 101 * m_delta_t)));
  m_targets.push_back(
      simulateTarget(this, arma::fvec({-200.f, 800.f}), arma::fvec({15.f, -5.f}),
                     std::make_pair(80 * m_delta_t, 101 * m_delta_t)));                                                                                                                              
}
